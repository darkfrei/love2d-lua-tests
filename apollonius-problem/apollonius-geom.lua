...
-- build line from two points (x1,y1)-(x2,y2)
...
-- recalc line coefficients and normalize
function geom.recalculateLine(l)
	...
	l.a, l.b, l.c = a, b, c
	geom.lineNormalization(l)
end

-- rebuild line: fill missing form (points or coefficients)
...
-- return length of line normal vector
function geom.lineNorm(l)
	return math.sqrt(l.a*l.a + l.b*l.b)
end

-- normalize line coefficients
function geom.lineNormalization(l)
	local length = geom.lineNorm(l)
	l.a = l.a / length
	l.b = l.b / length
	l.c = l.c / length
end

-- solve bisector equations where bisector is x = f(y)
function geom.solveBisectorX(p, l1, l2, bis, eps)
	eps = eps or epsylon
	geom.lineNormalization(l1)
	geom.lineNormalization(l2)
	local results = {}
	if math.abs(bis.a) < eps then return results end
	local m2 = -bis.b / bis.a
	local k2 = -bis.c / bis.a
	local px, py = p.x, p.y
	local Lc = (l1.a * m2 + l1.b)
	local Ld = (l1.a * k2 + l1.c)
	local A = m2*m2 + 1 - (Lc*Lc)
	local B = 2*(m2*(k2 - px) - py) - 2*(Lc*Ld)
	local C = (k2 - px)^2 + py^2 - (Ld*Ld)
...
-- solve bisector equations where bisector is y = f(x)
function geom.solveBisectorY(p, l1, l2, bis, eps)
	eps = eps or epsylon
	geom.lineNormalization(l1)
	geom.lineNormalization(l2)
	local results = {}
	if math.abs(bis.b) < eps then return results end
	local m = -bis.a / bis.b
	local k = -bis.c / bis.b
	local px, py = p.x, p.y
	...
	local A = 1 + m*m - (Lc*Lc)
	local B = -2*px + 2*m*(k - py) - 2*(Lc*Ld)
	local C = px*px + (k - py)^2 - (Ld*Ld)
...
-- choose which bisector form to solve depending on slope
function geom.solveBisector(p, l1, l2, bis, eps)
	if math.abs(bis.b) > math.abs(bis.a) then
		return geom.solveBisectorY(p, l1, l2, bis, eps)
	else
		return geom.solveBisectorX(p, l1, l2, bis, eps)
	end
end

-- build bisector line with sign s = ±1
...
-- solve 2x2 linear system with param t:
-- a1*x + b1*y = - (c0_1 + c1_1 * t)
-- a2*x + b2*y = - (c0_2 + c1_2 * t)
-- returns X0,X1,Y0,Y1,D  (x = X0 + X1*t, y = Y0 + Y1*t)
function geom.solveLinearSystemParam(a1,b1,a2,b2, c0_1,c0_2, c1_1,c1_2, eps)
	eps = eps or epsylon
	local D = a1*b2 - a2*b1
	local n1 = math.sqrt(a1*a1 + b1*b1)
	local n2 = math.sqrt(a2*a2 + b2*b2)
	if math.abs(D) < eps then
		return nil, ("determinant nearly zero: D=" .. tostring(D)), D, n1, n2
	end
	local X0 = (b1*c0_2 - b2*c0_1) / D
	local X1 = (b1*c1_2 - b2*c1_1) / D
	local Y0 = (a2*c0_1 - a1*c0_2) / D
	local Y1 = (a2*c1_1 - a1*c1_2) / D
...
function geom.circleCircleIntersection(c1, c2)
	...
	if d > c1.r + c2.r or d < math.abs(c1.r - c2.r) then return {} end
...
function geom.lineLineIntersection(l1, l2)
	...
	if math.abs(det) < epsylon then return nil end
...
function geom.perpendicularLine(l, p, flip)
	geom.recalculateLine(l)
	local a, b = -l.b, l.a
	if flip then a, b = -a, -b end
	local c = -(a*p.x + b*p.y)
	local perp = {a=a, b=b, c=c}
	geom.recalculateLine(perp)
	return perp
end

-- check if lines valid (non-degenerate)
function geom.areLinesValid(l1, l2, eps)
	eps = eps or epsylon
	local n1 = math.sqrt(l1.a^2 + l1.b^2)
	local n2 = math.sqrt(l2.a^2 + l2.b^2)
	if n1 < eps or n2 < eps then return false end
	return true, n1, n2
end

-- bisector from two normalized lines
function geom.lineBisector(l1, l2, n1, n2, s)
	return {
		a = l1.a/n1 + s * l2.a/n2,
		b = l1.b/n1 + s * l2.b/n2,
		c = l1.c/n1 + s * l2.c/n2
...
-- midpoint and perpendicular unit vector between two points
function geom.midpointAndPerpUnit(p1, p2)
	local dx = p2.x - p1.x
	local dy = p2.y - p1.y
	local d = math.sqrt(dx*dx + dy*dy)
	local m = {x=(p1.x+p2.x)/2, y=(p1.y+p2.y)/2}
	local u = {x=-dy/d, y=dx/d}
	return m, u, d
end

-- build quadratic coeffs for ppl case
function geom.pplQuadraticCoeffs(m, u, d, line)
	geom.recalculateLine(line)
	local a, b, c = line.a, line.b, line.c
	local N = a*a + b*b
	local Acoef = a*m.x + b*m.y + c
	local Bcoef = a*u.x + b*u.y
	local halfd = d/2
	local Qa = (Bcoef*Bcoef - N)
	local Qb = 2*Acoef*Bcoef
	local Qc = (Acoef*Acoef - N*(halfd*halfd))
	return Qa, Qb, Qc, N, Acoef, Bcoef
end

-- build solution from t, check circle validity
function geom.pplSolutionFromT(t, m, u, p1, line, eps)
	local cx = m.x + t * u.x
	local cy = m.y + t * u.y
	local center = {x=cx, y=cy}
	local r = geom.distance(center, p1)
	local dl = geom.pointLineDistance(center, line)
	local dp1 = geom.distance(center, p1)
	local valid = (math.abs(dl - r) < (eps or 1e-5)) and (math.abs(dp1 - r) < (eps or 1e-6))
