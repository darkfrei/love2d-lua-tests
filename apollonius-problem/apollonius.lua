...
-- ======================
-- finds all circles tangent to a given circle c and passing through points p1 and p2
function M.solveCPP(c, p1, p2)
	local solutions = {}

	-- get perpendicular line parameters through midpoint of p1-p2
	local mx, my, ux, uy, d = geom.linePerpendicularParam(p1, p2)
	if not mx then
		return solutions -- points coincide
	end

	local m = {x = mx, y = my}
	local u = {x = ux, y = uy}

	-- compute geometry parameters for tangency
	local d_half, dot_dc_perp, k = geom.tangencyParams(m, u, d, c)

	-- process both tangency types: external (sign=1) and internal (sign=-1)
	for _, sign in ipairs({1, -1}) do
		-- quadratic equation coefficients for parameter t
		local A_coef, B_coef, C_coef = geom.cppQuadraticCoeffs(dot_dc_perp, k, c.r, d_half)
		local roots = geom.solveQuadratic(A_coef, B_coef, C_coef)

		for _, t in ipairs(roots) do
			-- check root validity to remove false solutions
			if geom.validateTangencyRoot(t, dot_dc_perp, k, sign, c.r, d_half) then
				-- valid root found
				local circle = geom.circleFromPerpendicularParam(m, u, t, d_half)

				-- check that circle passes through both points
				if geom.validateCPPSolution(circle, circle.r, p1, p2) then
					table.insert(solutions, circle)
				end
			end
		end
	end
