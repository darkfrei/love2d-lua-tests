...
UI = {
	...
	DZETA = 0.05, -- damping tuning step per keypress
}
...
local function draw_status()
	...
	if gstate == "simulate" and sim_running then
		txt = string.format("Max: %.0f N t=%.2fs | %s",
...
local function draw_overlay()
	...
	if gstate == "simulate" and sim_running then
		...
		love.graphics.print(
			"DYNAMIC SIMULATION (Velocity Verlet, " ..
			world.config.SUBSTEPS .. " substeps) | [SPACE] = stop",
			UI.TBW + 10, 8)
	...
	elseif tool == "beam" and sel then
		...
		love.graphics.print("Node " .. sel .. " selected - click another. [ESC] cancel",
			UI.TBW + 10, 8)
...
---------------------------------------------------------------
-- Nearest-node query (UI helper)
---------------------------------------------------------------

-- finds nearest node to cursor within radius
function nearest_node_in_world(mx, my)
	local best_d2 = (UI.NR * 3)^2
	local best_i = nil
...
function love.mousepressed(mx, my, btn)
	...
	-- ignore clicks during simulation
...
function love.keypressed(k)
	...
	if k == "q" then cfg.ZETA_AXIAL = math.min(1, cfg.ZETA_AXIAL + UI.DZETA)
	elseif k == "e" then cfg.ZETA_AXIAL = math.max(0, cfg.ZETA_AXIAL - UI.DZETA)
	elseif k == "a" then cfg.ZETA_ANGULAR = math.min(1, cfg.ZETA_ANGULAR + UI.DZETA)
	elseif k == "d" then cfg.ZETA_ANGULAR = math.max(0, cfg.ZETA_ANGULAR - UI.DZETA)
	...
	elseif k == "escape" then sel = nil
	elseif k == "1" then tool = "node"; sel = nil
	elseif k == "2" then tool = "beam"; sel = nil
	elseif k == "3" then tool = "pin"; sel = nil
	elseif k == "4" then tool = "load"; sel = nil
	elseif k == "5" then tool = "del"; sel = nil
	end
end
