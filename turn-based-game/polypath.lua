...
-- based on https://github.com/darkfrei/love2d-lua-tests/tree/main/polypath
...
function PolyPath.new()
	...
	local self = setmetatable(path, PolyPath)
	self.points = {} 
	return self
end


-- find segment at specified distance
-- returns segment index, segment, and progress [0-1] within segment
function PolyPath:getSegmentAtDistance(targetDistance)
	local accumulatedDistance = 0
	for index, segment in ipairs(self.segments) do
		if accumulatedDistance + segment.length >= targetDistance then
			local segmentProgress = (targetDistance - accumulatedDistance) / segment.length
			return index, segment, segmentProgress
		end
		accumulatedDistance = accumulatedDistance + segment.length
	end
	-- if distance exceeds path length, return last segment
	local lastIndex = #self.segments
	return lastIndex, self.segments[lastIndex], 1
end


-- get point and angle at specified distance from path start
-- returns x, y, angle
function PolyPath:getPointAtDistance(distance)
	local index, segment, progress = self:getSegmentAtDistance(distance)

	-- get points for interpolation
	local p1 = self.points[index]
	local p2 = self.points[index + 1]

	-- interpolate position
	local x = p1.x + progress * (p2.x - p1.x)
	local y = p1.y + progress * (p2.y - p1.y)

	-- interpolate angle (with shortest path correction)
	local angle
	if p1.angle and p2.angle then
		local angleDiff = (p2.angle - p1.angle + math.pi) % (2 * math.pi) - math.pi
		angle = p1.angle + progress * angleDiff
	else
		-- fallback to segment direction if no angles stored
...
function PolyPath:calculateTotalLength()
	local length = 0
	for i = 1, #self.points - 1 do
		local p1 = self.points[i]
		local p2 = self.points[i + 1]
...
--- specific fturn based functions

-- constants
local ANGLE_STEP = math.rad(180/8) -- 22.5 degrees
...
local function calculateTurnRadius(segmentLength, angleStep)
	return segmentLength / math.tan(angleStep)
end
...
local function createPathPoint(x, y, angle)
	return {x = x, y = y, angle = angle}
end

local function createPathSegment(p1, p2)
	local length = calculateDistance(p1.x, p1.y, p2.x, p2.y)
	return {
		startX = p1.x,
		startY = p1.y,
		endX = p2.x,
...
local function smoothIntermediateAngles(points)
	for i = 2, #points-1 do
		local p1 = points[i-1]
		local p2 = points[i]
		local p3 = points[i+1]
		p2.angle = math.atan2(p3.y - p1.y, p3.x - p1.x)
	end
end

local function fixEndpointAngles(points, startAngle)
	if #points >= 2 then
		-- first point
		local p1 = points[1]
		local p2 = points[2]
		...
		-- last point
		local p3 = points[#points-1]
		local p4 = points[#points]
		p4.angle = math.atan2(p4.y - p3.y, p4.x - p3.x)
	end
end
...
-- calculates base turning circle from initial conditions
local function calculateBaseTurningCircle(x, y, angle, turnDirection, maxSegmentLength)
	-- generate three points along turning arc
	local p1 = {x = x, y = y}

	local angle2 = angle + turnDirection * ANGLE_STEP
	local p2 = {
		x = x + math.cos(angle2) * maxSegmentLength,
		y = y + math.sin(angle2) * maxSegmentLength
	}

	local angle3 = angle2 + turnDirection * ANGLE_STEP
	local p3 = {
		x = p2.x + math.cos(angle3) * maxSegmentLength,
		y = p2.y + math.sin(angle3) * maxSegmentLength
	}

	-- calculate circumcircle for these points
	local circumCircle = getCircumcircle(p1, p2, p3)
...
function PolyPath.calculatePath(startX, startY, targetX, targetY, startAngle, maxSegmentLength)
	local path = PolyPath.new()

	local x, y = startX, startY
	local angle = startAngle
--	local turnRadius = calculateTurnRadius(maxSegmentLength, ANGLE_STEP)

	-- add starting point
--    table.insert(path.points, createPathPoint(x, y, angle))
	path:addPoint(x, y)

	-- calculate initial direction to target
	local targetAngle = calculateDirection(x, y, targetX, targetY)
	local angleDiff = angleDifference(targetAngle, angle)
	local turnDirection = angleDiff > 0 and 1 or -1

	-- calculate base turning circle
	local baseCircle = calculateBaseTurningCircle(x, y, angle, turnDirection, maxSegmentLength)
	path.circle = {
		x = baseCircle.x,
		y = baseCircle.y,
	...
	-- check if target is inside turning circle before starting
--	local initialDist = calculateDistance(x, y, targetX, targetY)

	local initialDist = calculateDistance(baseCircle.x, baseCircle.y, targetX, targetY)
	local turnRadius = baseCircle.radius
	local turnAngle = calculateDirection(baseCircle.x, baseCircle.y, targetX, targetY)
	
	if initialDist < turnRadius then
		-- project target to turning circle boundary
		local newTargetX = baseCircle.x + math.cos(turnAngle) * turnRadius
		local newTargetY = baseCircle.y + math.sin(turnAngle) * turnRadius
	...
	while iterations < maxIterations do
		...
		local distanceToTarget = calculateDistance(x, y, targetX, targetY)
		...
		if distanceToTarget <= maxSegmentLength then
			-- add final point exactly at target
			path:addPoint(targetX, targetY)
--			print('adding final point', targetX, targetY)
			break
		end

		-- adjust movement angle based on target direction
		if math.abs(angleToTarget) > ANGLE_STEP then
			angle = angle + turnDirection * ANGLE_STEP
		else
			angle = directionToTarget
		end

		-- calculate next point position
		local newX = x + math.cos(angle) * maxSegmentLength
		local newY = y + math.sin(angle) * maxSegmentLength

		-- add new point to path
		path:addPoint(newX, newY)
--		print('adding point', newX, newY) -- ok

		-- update current position
		x, y = newX, newY
	end

	-- create segments and calculate total length
	path.totalLength = 0
	for i = 1, #path.points-1 do
		local segment = createPathSegment(path.points[i], path.points[i+1])
--		print('segment.length', segment.length) -- ok
		table.insert(path.segments, segment)
		path.totalLength = path.totalLength + segment.length
	end

	-- smooth angles between segments
	smoothIntermediateAngles(path.points)
	fixEndpointAngles(path.points, startAngle)


-- for test:
	for i = 2, #path.points-1 do
		local p1 = path.points[i-1]
		local p2 = path.points[i]
		local p3 = path.points[i+1]
		local circumCircle = getCircumcircle (p1, p2, p3)
		if circumCircle  then
--			print ('getCircumCircle', circumCircle.x, circumCircle.y, circumCircle.radius)
		end
	end
